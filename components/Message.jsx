    ...
    "self-end"  : ""}`}>
     {showDeletePopup && (
      ...
      />
         ...
         mb-1 ${self ? "justify-start flex-row-reverse" : ""}
         `}>
            <Avatar
             size="small"
            ...
            gap-4 p-4 rounded-3xl relative break-all ${self ? "rounded-br-md bg-c5" : 
            "rounded-bl-md bg-c1"}`}>
                {message.text &&(
                    <div
                     className='text-sm'
                ...
                group-hover:flex absolute top-2 ${self ? "left-2 bg-c5" : 
                "right-2 bg-c1"}`}
         ...
         <div className={`flex items-end ${self ? "justify-start flex-row-reverse mr-12" :
        "ml-12"}`}>
             <div className='text-xs
             text-c3'>{formateDate(date)}</div>
        </div>
    </div>
  )
}
