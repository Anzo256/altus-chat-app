...
const ChatMenu = ({showMenu, setShowMenu}) => {
      ...
      const isUserBlocked = users[currentUser.uid]?.blockedUsers?.
      find(u => u === data.user.uid);

      const IamBlocked = users[data.user.uid]?.blockedUsers?.
      find(u => u === currentUser.uid);
      ...
      const handleDelete = async() =>{
          try {
              ...
              const updateMessages = chatDoc.data().messages.map
              (message => {
             ...
             .filter(([id, chats])=> id !== data.chatId)
  ...
  return (
        ...
        <div className='w-[200px] absolute 
              top-[70px] right-5 bg-c0 z-10
               rounded-md overflow-hidden'>
           <ul className='flex flex-col py-2'>

              {!IamBlocked && (<li className='flex 
                ...
                >{isUserBlocked ? "Unblock" : "Block user"}</li>
              ) }
